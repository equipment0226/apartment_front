...
import {
  ...
  ComposedChart, Area, Legend, Scatter, ScatterChart, ZAxis,
} from "recharts";
...
// 공통: 아파트 선택 필터
// ---------------------------------------------------------------------------
function ApartmentFilter({ aptGu, setAptGu, aptDong, setAptDong, aptComplex, setAptComplex, aptBuilding, setAptBuilding, filters, filterError }) {
  return (
    <>
      ...
      <div className="sim-apartment-grid">
        ...
        <label className="sim-field">
          <span>구</span>
          <select value={aptGu} onChange={(e) => { setAptGu(e.target.value); setAptDong(""); setAptComplex(""); setAptBuilding(""); }}>
        ...
        <label className="sim-field">
          <span>동</span>
          <select value={aptDong} onChange={(e) => { setAptDong(e.target.value); setAptComplex(""); setAptBuilding(""); }}>
            <option value="">선택</option>
        ...
        <label className="sim-field">
          <span>단지명</span>
          <select value={aptComplex} onChange={(e) => { setAptComplex(e.target.value); setAptBuilding(""); }}>
            <option value="">선택</option>
      ...
      <p className="sim-meta-inline">기준시세(최근 실거래): <strong>{formatManwon(filters.base_price_manwon)}</strong> · 매칭 {filters.matched_rows || 0}건</p>
    </>
  );
}
...
// ---------------------------------------------------------------------------
function useApartmentFilters(aptGu, aptDong, aptComplex, aptBuilding) {
  const [filters, setFilters] = useState({
    si_options: ["서울특별시"], gu_options: [], dong_options: [], complex_options: [], building_options: [],
  ...
  }, [aptGu, aptDong, aptComplex, aptBuilding]);
...
function UserAnalysisTab() {
  ...
  const { filters, filterError } = useApartmentFilters(aptGu, aptDong, aptComplex, aptBuilding);
    ...
    <div className="sim-grid">
      ...
      <div className="sim-panel">
        ...
        <label className="sim-field" style={{ marginTop: 12 }}>
          ...
          <input
            type="range" min={1} max={240} value={targetMonths}
      ...
      <div className="sim-panel sim-result">
        ...
        {!result ? (
          ...
          <>
            ...
            <StorytellingReport
              result={result}
              apt={{ si: "서울특별시", gu: aptGu, dong: aptDong, complex_name: aptComplex, building: aptBuilding }}
...
// ────────────────────────────────────────────────────────────────────────
function FiveYearFanChart({ forecast, basePriceManwon, p10Ret, p50Ret, p90Ret, targetMonths, transactions }) {
  const data = useMemo(
  ...
  // 과거 36개월 실거래 산점 + 미래 fan 을 하나의 시계열로 합치기
  const merged = useMemo(() => {
    const past = (transactions || []).map((t) => ({
      t: String(t.date || t.timestamp || "").slice(0, 7),
      tx_eok: Number(t.price_manwon) / 10000,
    })).filter((r) => Number.isFinite(r.tx_eok));
    const fut = data.map((d) => ({ ...d }));
    return [...past, ...fut];
  }, [transactions, data]);

  if (!merged.length) return <p className="sim-placeholder">예측 데이터가 없습니다.</p>;
  ...
  return (
    <div className="story-fan-wrap">
      <ResponsiveContainer width="100%" height={300}>
        <ComposedChart data={merged} margin={{ top: 16, right: 24, bottom: 8, left: 8 }}>
            ...
            content={({ active, payload, label }) => {
              ...
              return (
                <div className="story-tooltip">
                  ...
                  {row.tx_eok != null && (
                    <p>실거래: <strong>{row.tx_eok.toFixed(2)}억</strong></p>
                  )}
                  {row.median_eok != null && (
                    <>
          ...
          <Legend
            ...
            payload={[
              { value: "과거 3년 실거래", type: "circle", color: "#f77f00" },
              { value: "예상 가격 (중앙값 · p50)", type: "line", color: "#4361ee" },
              { value: "예상 범위 (p10 ~ p90)", type: "rect", color: "#4361ee55" },
            ]}
          />
          {/* p10~p90 음영 밴드 (라인 아래로 깔리도록 먼저 렌더) */}
          <Area type="monotone" dataKey="band_eok" stroke="none" fill="#4361ee" fillOpacity={0.22} connectNulls={false} />
          {/* 중앙값(p50) 라인 — 가장 위 */}
          <Line
          ...
          />
          {/* 과거 실거래 산점 */}
          <Scatter dataKey="tx_eok" fill="#f77f00" shape="circle" />
          {/* 마일스톤 라벨 점 */}
          <Line
...
정리에 필요한 핵심 요소 4가지:
  ...
  2) 거시 변수 (기준금리, 주담대 금리, CD금리, 전세가격, CPI/물가, 실업률, M2 통화량,
     LTV/DSR 규제, 투기지역·조정지역 등 부동산 정책, 인구·세대수 등)
  3) 방향 (상승/하락/완화/강화)
  4) 크기 또는 정도 (예: 0.5%p, 5%, 큰 폭, 소폭)

⚠ 절대 묻지 말아야 할 것:
- "집값(주택 매매가격)이 얼마나 오를/내릴까요?" 같이 **집값(주택 매매가격) 자체**는 입력이 아니라
  본 시뮬레이션이 예측해서 알려줄 결과(타깃)입니다. 따라서 집값/주택가격/아파트값에 대한
  사용자 가정은 절대 묻지 마세요. 대신 위 2)의 거시 변수에 대한 가정만 수집하세요.

진행 규칙:
- 사용자 입력이 모호하거나 4가지 요소 중 빠진 게 있으면, 빠진 부분을 콕 집어 한 가지만 친근하게 되묻습니다.
  반드시 모범 예시 형태를 함께 보여주세요. 예: "2026년 9월에 기준금리가 0.5%p 인상되면 어떻게 될까요?"
- 사용자가 집값/주택가격 자체에 대한 가정을 말하면, 정중히 "집값은 저희가 예측해 드리는 값이라
  직접 가정으로 받지 않습니다. 대신 기준금리·전세·정책 등 어떤 거시 환경을 가정할지 알려주세요"
  라고 안내한 뒤 거시 변수를 한 가지만 되묻습니다.
...
반드시 아래 JSON만 반환하세요 (코드펜스/설명 금지):
{
  ...
  "synthesized_query": "status=complete일 때만 채움. 시점·거시변수·방향·크기가 들어간 자연어 한 문장. 집값/주택가격은 절대 포함하지 말 것. 예: '2026년 9월에 기준금리가 0.5%p 인상되면 어떻게 될까?'"
}`;
    ...
    <div className="story-report">
      ...
      <section className="story-section">
        ...
        <FiveYearFanChart
          ...
          targetMonths={targetMonths}
          transactions={result.transactions_3y}
        />
        {milestones.length > 0 && (
